...
class App extends Component {
  ...
  render() {
    const { value, selection, showOutput, showResult } = this.state
    ...
    return (
      <div className={isProd ? 'demo-container' : undefined}>
        ...
        <div style={{ marginTop: 10 }}>
          ...
          <span className="arrow">{!showOutput ? ' ▶' : ' ▼'}</span>
        </div>

        {showOutput && <pre className="select-text">{value}</pre>}

        <div style={{ marginTop: 10 }}>
          <a href="" onClick={this.toggleResult}>
            {`${showResult ? 'Hide' : 'Show'} result`}
          </a>
          <span className="arrow">{!showResult ? ' ▶' : ' ▼'}</span>
        </div>

        {showResult && <div className="select-text" dangerouslySetInnerHTML={{ __html: value }} />}
